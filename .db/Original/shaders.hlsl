...
#define MyRS1 "RootFlags( ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT), " \
              ...
              "DescriptorTable(Sampler(s0, numDescriptors = 6))"

static const float2 standardInvScreenSize = float2(1.f / 1600.f, 1.f / 900.f);
...
LowPPixelProcessOutput lowP_DoGlow(PixelProcessInput input) {
	float4 textureSample = texture0.Sample(smoothWrappingSamplerState, input.tex);
...
LowPPixelProcessOutput mainp_lowP_Process_DeferredTextureColor_EmissiveGlowBrightness_PS(PixelProcessInput input) {
	LowPPixelProcessOutput pixelProcessOutput = lowP_DoGlow(input);
...
static const float NOISE_WEIGHT = .7f;
static const float NOISE_MARGIN = .03f;
static const float NOISE_OUTLINE = .025f;
static const float NOISE_OUTLINE_THRESHOLD = .015f;
static const float3 STONE_BASE_COLOR = float3(.35f, .37f, .4f);
static const float3 STONE_OUTLINE_COLOR = float3(1.5f, 1.55f, 1.5f);

PixelProcessOutput mainp_Process_DeferredTexture_Stone_PS(PixelProcessInput input) {
	float4 textureSample = texture0.Sample(smoothWrappingSamplerState, input.tex);
	float luma = GetLuma(textureSample.rgb);
	...
	float stoneNoise = noise * .8f + .2f;
	float3 finalColor = STONE_BASE_COLOR * luma * stoneNoise;
	if (input.color.r != 0.f) {
		...
		if (luma > NOISE_OUTLINE_THRESHOLD && input.color.r > 0.f && input.color.r < noise && input.color.r > noise - NOISE_OUTLINE) {
			// Conversion frontier color
			finalColor = STONE_OUTLINE_COLOR * luma;
		}
	}
...
LowPPixelProcessOutput mainp_lowP_Process_DeferredTexture_Stone_PS(PixelProcessInput input) {
	float4 textureSample = texture0.Sample(smoothWrappingSamplerState, input.tex);
	float luma = GetLuma(textureSample.rgb);

	float2 worldPosition = GetWorldPosition(input.position.xy * invScreenSize).xy;
	float noise = texture1.Sample(smoothWrappingSamplerState, worldPosition / float2(5.5f, 5.5f)).r;
	noise = NOISE_MARGIN + noise * (1.f - NOISE_MARGIN);
...
float4 ProcessNebula(PixelProcessInput input) {
...
float4 LiquidHelperWater(float2 tex, float2 noise, float currentLiquidSample, float4 liquidSample) {
...
LiquidOutput mainp_LiquidWater_PS(PixelInputType input) {
	LiquidOutput output;
	...
	float4 liquidOutput = LiquidHelperWater(input.tex, noise, currentLiquidSample, float4(.08f, .69f, .89f, .43f));
...
float4 mainp_lowp_LiquidWater_PS(PixelInputType input) : SV_TARGET0 {
	float4 output;
...
LiquidOutput mainp_LiquidEggplant_PS(PixelInputType input) {
	...
	float4 liquidOutput = LiquidHelperWater(input.tex, noise, currentLiquidSample, float4(.98f, .05f, .64f, .43f));
	output.liquid = pow(liquidOutput, GAMMA_CORRECTION);
	output.emissive = float4(0.f, 0.f, 0.f, 0.f);
...
float4 LiquidHelperLava(float currentLiquidSample) {
	float alpha = saturate((1.f / (LIQUID_END_CUTOFF - LIQUID_START_CUTOFF)) * currentLiquidSample - (LIQUID_START_CUTOFF / (LIQUID_END_CUTOFF - LIQUID_START_CUTOFF)));
	if (alpha == 0.f) {
		// Air
		discard;
	...
	return float4(1.f, .56f, 0.f, alpha);
}

LiquidOutput mainp_LiquidLava_PS(PixelInputType input) {
	LiquidOutput output;

	float currentLiquidSample = texture0.Sample(smoothBorderSamplerState, input.tex).g;

	float4 liquidOutput = LiquidHelperLava(currentLiquidSample);
...
float4 mainp_lowp_LiquidLava_PS(PixelInputType input) : SV_TARGET0 {
	float4 output;

	float currentLiquidSample = texture0.Sample(smoothBorderSamplerState, input.tex).g;

	float4 liquidOutput = LiquidHelperLava(currentLiquidSample);
	output = float4(pow(liquidOutput.rgb, GAMMA_CORRECTION), liquidOutput.a);

	float sampleHotness = pow(currentLiquidSample, 4.f);
	float3 emissiveTint = TemperatureHelper(3000.f * sampleHotness);
	output += float4(.4f * liquidOutput.a * emissiveTint, liquidOutput.a);
...
#define AMBIENTOCCLUSION_UTEXEL (standardInvScreenSize.x * pow(2.f, AMBIENTOCCLUSION_SHRINK_FACTOR))
#define AMBIENTOCCLUSION_VTEXEL (standardInvScreenSize.y * pow(2.f, AMBIENTOCCLUSION_SHRINK_FACTOR))
...
float4 mainp_SoftShadower_PS(PixelInputType input) : SV_TARGET0 {
	float2 depthSamplingDirection = -standardInvScreenSize;
...
float4 mainp_RimLight_PS(PixelLightingInput input) : SV_TARGET0 {
	...
	if (ownDepthSample >= RIM_MIN_DEPTH && ownDepthSample <= RIM_MAX_DEPTH && ownDepthSample != RIM_IGNORE_DEPTH) {
		...
		float2 directionVector = normalize(input.tex) * standardInvScreenSize * RIM_SAMPLE_DELTA;
...
static const float bayerMatrix[8][8] = {
	{ 0.f,		   .501960814f, .149019614f, .627451003f, .062745101f, .533333361f, .176470593f, .658823550f },
	{ .749019623f, .258823544f, .878431380f, .376470596f, .784313738f, .290196091f, .913725495f, .407843143f },
	{ .200000003f, .690196097f, .086274512f, .556862772f, .227450982f, .721568644f, .117647059f, .596078455f },
	{ .945098042f, .435294122f, .811764717f, .313725501f, .976470590f, .466666669f, .847058833f, .345098048f },
	{ .078431375f, .545098066f, .184313729f, .670588255f, .039215687f, .517647088f, .160784319f, .639215708f },
	{ .800000012f, .301960796f, .929411769f, .419607848f, .768627465f, .274509817f, .898039222f, .392156869f },
	{ .247058824f, .737254918f, .129411772f, .607843161f, .215686277f, .701960802f, .105882354f, .572549045f },
	{ .996078432f, .482352942f, .862745106f, .360784322f, .956862748f, .454901963f, .831372559f, .333333343f }
};

float3 PosterizeDithering(float3 color, float2 position) {
	int positionInt[2] = { int(position.x), int(position.y) };
	float ofs = bayerMatrix[positionInt[1] % BAYER_SIZE][positionInt[0] % BAYER_SIZE] - .5f;
	float chCount = (1 << 8) - 1.f;
...
float4 mainp_LightingPost_PS(PixelInputType input) : SV_TARGET0 {
	...
	float3 toneColor = saturate(PosterizeDithering(postLight, input.position.xy));
	float3 result = lerp(Grading_Helper(toneColor, texture5), Grading_Helper(toneColor, texture6), LUTInterpolation) * vignetteStrength;
	return float4(result, 1.f);
}

float4 mainp_lowp_LightingPost_PS(PixelInputType input) : SV_TARGET0 {
	float3 diffuseTextureSample = texture0.Sample(sharpClampingSamplerState, input.tex).rgb;
	float3 multiplicativeLightEffect = texture1.Sample(smoothClampingSamplerState, input.tex).rgb;
	float3 additiveLightEffect = texture2.Sample(smoothClampingSamplerState, input.tex).rgb;
	float emissiveFactor = 1.f - texture0.Sample(sharpClampingSamplerState, input.tex).a;
	...
	float3 diffuseLinear = max(1.055f * pow(diffuseTextureSample.rgb, 1.0f / 2.4f) - 0.055f, 0.f);
	float3 invGammaCorrected = pow(sqrt(diffuseLinear) * multiplicativeLightEffect, 2.f);
	float3 postLight = lerp(invGammaCorrected, diffuseLinear, emissiveFactor) + additiveLightEffect;
...
float4 mainp_LayerSingle_PS(PixelInputType input) : SV_TARGET0 {
	float4 layerSample = texture0.Sample(sharpClampingSamplerState, input.tex);
	
	layerSample.rgb *= brightness;

	return layerSample;
}

cbuffer LayerMergerCBufferType : register(b1) {
...
float3 UIProcess(PixelInputType input) {
...
float4 mainp_UIProcess_PS(PixelInputType input) : SV_TARGET0 {
	float3 diffuse = UIProcess(input);

	return float4(diffuse, 1.f);
}

float4 mainp_lowp_UIProcess_PS(PixelInputType input) : SV_TARGET0 {
	float shadowSamples = texture1.Sample(sharpClampingSamplerState, input.tex).r;

	float shadow = 1.f - shadowSamples * (1.f - UISHADOW_OPACITY);
