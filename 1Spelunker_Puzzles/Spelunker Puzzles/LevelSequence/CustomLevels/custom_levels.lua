...
local function load_level(file_name, width, height, load_level_ctx, allowed_spawn_types)
    ...
    custom_level_state.bat_callback = set_post_entity_spawn(function(entity, spawn_type)
        if test_flag(custom_level_state.allowed_spawn_types, ALLOW_SPAWN_TYPE.LITWALLTORCH) then return end
        -- Do not remove spawns from a script.
        if spawn_type & SPAWN_TYPE.SCRIPT ~= 0 then return end
        entity.flags = set_flag(entity.flags, ENT_FLAG.INVISIBLE)
        entity.flags = set_flag(entity.flags, ENT_FLAG.DEAD)
        entity:destroy()
    end, SPAWN_TYPE.LEVEL_GEN_GENERAL, 0, ENT_TYPE.ITEM_LITWALLTORCH)
